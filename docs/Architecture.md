...
│      │          │          │          │             │
│  ┌───▼───┐ ┌───▼────┐ ┌───▼────┐ ┌───▼────┐ ┌───▼────┐ ┌───▼────┐ │
│  │ Login │ │Register│ │  Home  │ │ Admin  │ │Model3D │ │Construc│ │
│  │ Page  │ │  Page  │ │  Page  │ │  Page  │ │  Page  │ │tor Page│ │
│  └───┬───┘ └───┬────┘ └───┬────┘ └───┬────┘ └───┬────┘ └───┬────┘ │
│      │         │          │          │          │          │       │
│      └─────────┴──────────┴──────────┴──────────┴──────────┘  │
│                    │                                │
...
1. **App.jsx**
   ...
   - Определение маршрутов: `/login`, `/register`, `/home`, `/admin`, `/model`, `/constructor`
...
3. **Home.jsx** (в папке `pages/Home/`)
   - Главная страница с 3D сценой и прокруткой
   - Использует React Three Fiber для 3D визуализации
   - Компоненты:
     - `HeaderMain.jsx` - шапка с навигацией и логотипом
     - `Overlay.jsx` - оверлей с текстовым контентом
     - `Bubble.jsx` - обертка для 3D сцены с Environment
     - `Two.jsx` - основной 3D компонент с моделью яхты и управлением фоном
   - Анимация фона при прокрутке (светло-серый → грязно-серый)
   - ScrollControls для плавной прокрутки
...
5. **Model3D.jsx**
   ...
   - Leva панель для управления фоном (только для администраторов)
...
8. **upload.service.js**
   ...
   - Методы: `uploadFile()`, `getFiles()`, `deleteFile()`, `getGltfBackground()`, `updateGltfBackground()`
   - Работа с FormData для загрузки файлов
   - Работа с GLTF файлами (чтение/запись background данных)
   - Передача JWT токена в заголовках
...
9. **jwt.utils.js**
   ...
   - `isAdmin()` - проверка админских прав
   - `getUsername()` - получение имени пользователя из токена
...
│  ├─ Register.jsx
│  ├─ Home/
│  │  ├─ Home.jsx          # Главная страница (бывший LayoutMain)
│  │  ├─ HeaderMain.jsx    # Шапка с навигацией
│  │  ├─ Overlay.jsx       # Оверлей с текстовым контентом
│  │  ├─ Bubble.jsx        # Обертка для 3D сцены
│  │  └─ Two.jsx           # 3D компонент с моделью и управлением фоном
│  ├─ Admin.jsx
│  ├─ Model3D.jsx
│  └─ Constructor/
