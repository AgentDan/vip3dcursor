...
const updateGltfEnv = async (req, res) => {
  try {
    ...
    const result = await uploadService.updateGltfEnv(filePath, env);
    res.status(200).json(result);
  } catch (error) {
    console.error('Update GLTF env error:', error);
    res.status(500).json({ error: error.message || 'Failed to update GLTF env' });
  }
};
