...
function Admin() {
  ...
  const [activeTab, setActiveTab] = useState('users'); // 'users', 'files', or 'laboratory'
  ...
  const [uploadLabFile, setUploadLabFile] = useState(null);
  const [uploadLabFileLoading, setUploadLabFileLoading] = useState(false);
  const [uploadLabUploadLoading, setUploadLabUploadLoading] = useState(false);
  const [uploadLabFileData, setUploadLabFileData] = useState(null);
  const [uploadLabEnvTypes, setUploadLabEnvTypes] = useState(null);
  const [uploadLabEnvTypesLoading, setUploadLabEnvTypesLoading] = useState(false);
  const [uploadLabEnvStructure, setUploadLabEnvStructure] = useState(null);
  const [uploadLabEnvStructureLoading, setUploadLabEnvStructureLoading] = useState(false);
  ...
  // Загрузка информации о файле uploadlab
  const fetchUploadLabFile = async () => {
    setUploadLabFileLoading(true);
    try {
      const response = await uploadService.getUploadLabFile();
      if (response && response.file) {
        setUploadLabFileData(response.file);
        setError('');
      } else {
        setUploadLabFileData(null);
        setError('');
      }
    } catch (err) {
      setError(err.message);
      ...
      setUploadLabFileLoading(false);
    }
  };

  // Загрузка файла в uploadlab
  const handleUploadLabFileUpload = async (e) => {
    e.preventDefault();
    setError('');
    setUploadLabUploadLoading(true);

    if (!uploadLabFile) {
      setError('Please select a file');
      setUploadLabUploadLoading(false);
      return;
    }

    try {
      const result = await uploadService.uploadUploadLabFile(uploadLabFile);
      
      // Сбрасываем input файла
      const fileInput = document.getElementById('uploadlab-file-input');
      if (fileInput) {
        fileInput.value = '';
      }
      
      setUploadLabFile(null);
      
      // Используем данные из ответа сервера для немедленного отображения
      if (result && result.file) {
        setUploadLabFileData({
          filename: result.file.filename,
          size: result.file.size,
          url: result.file.url,
          originalName: result.file.originalName,
          createdAt: result.file.createdAt || new Date(),
          modifiedAt: result.file.modifiedAt || new Date()
        });
        
        // Если это GLTF файл, загружаем типы env и структуру
        if (result.file.filename && result.file.filename.endsWith('.gltf')) {
          await fetchUploadLabEnvTypes();
          await fetchUploadLabEnvStructure();
        }
      } else {
        // Если данных нет в ответе, обновляем информацию о файле
        await fetchUploadLabFile();
        // Если это GLTF файл, загружаем типы env
        if (uploadLabFileData && uploadLabFileData.filename && uploadLabFileData.filename.endsWith('.gltf')) {
          await fetchUploadLabEnvTypes();
        }
      }
    } catch (err) {
  ...
  // Обработка выбора файла для uploadlab
  const handleUploadLabFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      setUploadLabFile(file);
    }
  };

  // Удаление файла из uploadlab
  const handleDeleteUploadLabFile = async () => {
    if (!window.confirm('Are you sure you want to delete the uploadlab file? This action cannot be undone.')) {
      return;
    }
  ...
  // Скачивание файла из uploadlab
  const handleDownloadUploadLabFile = () => {
    if (!uploadLabFileData || !uploadLabFileData.url) {
      setError('File URL is not available');
      return;
    }
    const fileUrl = getFileUrl(uploadLabFileData.url);
    if (!fileUrl) {
  ...
  // Загрузка типов из env объекта GLTF файла
  const fetchUploadLabEnvTypes = async () => {
    // Проверяем наличие файла перед запросом
    if (!uploadLabFileData || !uploadLabFileData.filename || !uploadLabFileData.filename.endsWith('.gltf')) {
      setUploadLabEnvTypes(null);
      return;
    }
    
    setUploadLabEnvTypesLoading(true);
    try {
      const result = await uploadService.getUploadLabGltfEnvTypes();
      setUploadLabEnvTypes(result);
      setError('');
    } catch (err) {
      setError(err.message);
      setUploadLabEnvTypes(null);
    } finally {
      setUploadLabEnvTypesLoading(false);
    }
  };

  // Загрузка структуры env из GLTF файла
  const fetchUploadLabEnvStructure = async () => {
    // Проверяем наличие файла перед запросом
    if (!uploadLabFileData || !uploadLabFileData.filename || !uploadLabFileData.filename.endsWith('.gltf')) {
      setUploadLabEnvStructure(null);
      return;
    }
    
    setUploadLabEnvStructureLoading(true);
    try {
      const result = await uploadService.getUploadLabGltfEnvStructure();
      setUploadLabEnvStructure(result);
      setError('');
    } catch (err) {
      setError(err.message);
      setUploadLabEnvStructure(null);
    } finally {
      setUploadLabEnvStructureLoading(false);
    }
  };

  // Автоматически загружаем env данные при изменении файла
  useEffect(() => {
    if (uploadLabFileData && uploadLabFileData.filename && uploadLabFileData.filename.endsWith('.gltf')) {
      fetchUploadLabEnvTypes();
      fetchUploadLabEnvStructure();
    } else {
      // Очищаем данные, если файл не GLTF или отсутствует
      setUploadLabEnvTypes(null);
      setUploadLabEnvStructure(null);
    }
  }, [uploadLabFileData]);
              ...
              {activeTab === 'laboratory' && (
                <div className="mb-6 sm:mb-8">
                  <div className="mb-4 sm:mb-6">
                    <h2 className="text-2xl sm:text-3xl font-light text-gray-900 tracking-tight">Laboratory</h2>
                  </div>

                  <div className="space-y-6">
                    {/* File Upload Section - Compact */}
                    <div className="bg-white/40 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-gray-200/30">
                      {uploadLabFileLoading ? (
                        <div className="text-center py-8">
                          <div className="inline-block animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-blue-600 mb-2"></div>
                          <p className="text-gray-600 text-sm">Loading...</p>
                        </div>
                      ) : (
                        <form onSubmit={handleUploadLabFileUpload} className="space-y-2">
                          {/* Single row: Upload button, file name, Download/Delete buttons */}
                          <div className="flex items-center gap-2">
                            {/* File selection button */}
                            <label className="flex items-center justify-center h-8 w-8 border border-gray-300/50 border-dashed rounded cursor-pointer bg-white/40 backdrop-blur-sm hover:bg-white/60 transition-colors flex-shrink-0">
                              <svg className="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                              </svg>
                              <input
                                id="uploadlab-file-input"
                                type="file"
                                className="hidden"
                                onChange={handleUploadLabFileChange}
                              />
                            </label>
                            ...
                            <button
                              type="submit"
                              disabled={uploadLabUploadLoading || !uploadLabFile}
                              className="px-2.5 py-1.5 bg-gray-900 text-white rounded text-[10px] font-light uppercase tracking-wider hover:bg-gray-800 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center flex-shrink-0"
                            >
                              {uploadLabUploadLoading ? (
                            ...
                            {uploadLabFileData && uploadLabFileData.filename ? (
                              <>
                                <div className="flex items-center flex-1 min-w-0 bg-white/60 backdrop-blur-sm rounded px-2 py-1 border border-gray-200/30">
                                  <div className="flex-shrink-0 h-5 w-5 bg-gradient-to-br from-blue-400 to-indigo-500 rounded flex items-center justify-center text-white font-semibold mr-1.5 text-[9px]">
                                    {uploadLabFileData.filename.split('.').pop()?.toUpperCase().slice(0, 3) || 'FILE'}
                                  </div>
                                  <div className="min-w-0 flex-1">
                                    <div className="text-xs font-medium text-gray-900 truncate">{uploadLabFileData.filename}</div>
                                    <div className="text-[9px] text-gray-500">{formatFileSize(uploadLabFileData.size)}</div>
                                  </div>
                                </div>
                                <button
                                  onClick={handleDownloadUploadLabFile}
                                  className="px-2 py-1 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 transition-all font-medium text-xs inline-flex items-center cursor-pointer flex-shrink-0"
                                  title="Download file"
                                >
                                ...
                                <button
                                  onClick={handleDeleteUploadLabFile}
                                  className="px-2 py-1 bg-red-100 text-red-700 rounded hover:bg-red-200 transition-all font-medium text-xs inline-flex items-center cursor-pointer flex-shrink-0"
                                  title="Delete file"
                                >
                                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                              ...
                              <div className="flex-1 px-2 py-1 bg-white/60 backdrop-blur-sm text-gray-700 rounded text-[9px] font-light border border-gray-300/30 truncate">
                                {uploadLabFile.name} ({(uploadLabFile.size / 1024 / 1024).toFixed(2)} MB)
                              </div>
                            ) : null}
                          </div>
                        </form>
                    ...
                    {/* GLTF Env Types Table - At the bottom */}
                    {uploadLabFileData && uploadLabFileData.filename && uploadLabFileData.filename.endsWith('.gltf') && (
                      <div className="bg-white/40 backdrop-blur-sm rounded-lg p-6 sm:p-8 border border-gray-200/30">
                        <div className="flex items-center justify-between mb-6">
                          <h3 className="text-xl sm:text-2xl font-light text-gray-900 tracking-tight">GLTF Env Types</h3>
                          <button
                            onClick={fetchUploadLabEnvTypes}
                            disabled={uploadLabEnvTypesLoading}
                            className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded hover:bg-gray-200 transition-all font-medium text-xs inline-flex items-center cursor-pointer disabled:opacity-50"
                            title="Refresh env types"
                          >
                        ...
                        {uploadLabEnvTypesLoading ? (
                          <div className="text-center py-12">
                            <div className="inline-block animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-600 mb-4"></div>
                            <p className="text-gray-600 text-lg">Loading env types...</p>
                          </div>
                        ) : uploadLabEnvTypes && !uploadLabEnvTypes.error ? (
                          <div className="space-y-4">
                            <div className="bg-white/60 backdrop-blur-sm rounded-lg p-4 border border-gray-200/30 mb-4">
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="text-sm text-gray-600 font-light">File: <span className="font-medium text-gray-900">{uploadLabEnvTypes.filename}</span></p>
                                  <p className="text-sm text-gray-600 font-light mt-1">Total env items: <span className="font-medium text-gray-900">{uploadLabEnvTypes.totalEnvItems}</span></p>
                                </div>
                              </div>
                            </div>

                            {uploadLabEnvTypes.fullEnv && uploadLabEnvTypes.fullEnv.length > 0 ? (
                              <div className="overflow-x-auto">
                                <table className="w-full border-collapse">
                                  <thead>
                                    <tr className="border-b border-gray-300/50">
                                      <th className="px-4 py-3 text-left text-xs font-light uppercase tracking-wider text-gray-600 bg-white/40">Index</th>
                                      <th className="px-4 py-3 text-left text-xs font-light uppercase tracking-wider text-gray-600 bg-white/40">Type</th>
                                      <th className="px-4 py-3 text-left text-xs font-light uppercase tracking-wider text-gray-600 bg-white/40">Details</th>
                                    </tr>
                                  </thead>
                                  <tbody className="bg-white/20">
                                    {uploadLabEnvTypes.fullEnv.map((item, index) => (
                                      item && item.type ? (
                                        <tr key={index} className="border-b border-gray-200/30 hover:bg-white/30 transition-colors">
                                          ...
                                          <td className="px-4 py-3 text-sm text-gray-700">
                                            <details className="cursor-pointer">
                                              <summary className="text-xs text-gray-500 hover:text-gray-700 font-light">
                                                View details
                                              </summary>
                                              <div className="mt-2">
                                                <div className="bg-gray-50/60 rounded p-3 border border-gray-200/30">
                              ...
                              <div className="bg-gray-50/60 backdrop-blur-sm rounded-lg p-4 border border-gray-200/30 text-center">
                                <p className="text-gray-500 text-sm font-light">No types found in env array</p>
                              </div>
                            )}
                          </div>
                        ) : uploadLabEnvTypes && uploadLabEnvTypes.error ? (
                    ...
                    {/* GLTF Env Structure Section - At the bottom */}
                    {uploadLabFileData && uploadLabFileData.filename && uploadLabFileData.filename.endsWith('.gltf') && (
                      <div className="bg-white/40 backdrop-blur-sm rounded-lg p-6 sm:p-8 border border-gray-200/30">
                        <div className="flex items-center justify-between mb-6">
                          <h3 className="text-xl sm:text-2xl font-light text-gray-900 tracking-tight">GLTF Env Structure</h3>
                          <button
                            onClick={fetchUploadLabEnvStructure}
                            disabled={uploadLabEnvStructureLoading}
                            className="px-3 py-1.5 bg-gray-100 text-gray-700 rounded hover:bg-gray-200 transition-all font-medium text-xs inline-flex items-center cursor-pointer disabled:opacity-50"
                            title="Refresh env structure"
                          >
                            <svg className="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                            </svg>
                        ...
                        {uploadLabEnvStructureLoading ? (
                          <div className="text-center py-12">
                            <div className="inline-block animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-600 mb-4"></div>
                            <p className="text-gray-600 text-lg">Loading env structure...</p>
                          </div>
                        ) : uploadLabEnvStructure && !uploadLabEnvStructure.error ? (
                          <div className="space-y-4">
                            <div className="bg-white/60 backdrop-blur-sm rounded-lg p-4 border border-gray-200/30 mb-4">
                              <div className="flex items-center justify-between">
                                <div>
                                  <p className="text-sm text-gray-600 font-light">File: <span className="font-medium text-gray-900">{uploadLabEnvStructure.filename}</span></p>
                                  <p className="text-sm text-gray-600 font-light mt-1">Env items: <span className="font-medium text-gray-900">{uploadLabEnvStructure.envCount}</span></p>
                                </div>
                              </div>
                            </div>

                            <div className="bg-white/60 backdrop-blur-sm rounded-lg p-4 border border-gray-200/30">
                              <h4 className="text-sm font-medium text-gray-900 mb-3">Full Env Structure:</h4>
                              <div className="bg-gray-900 rounded-lg p-4 overflow-x-auto">
                                <pre className="text-xs text-gray-100 font-mono whitespace-pre-wrap">
                                  {JSON.stringify(uploadLabEnvStructure.envStructure, null, 2)}
                                </pre>
                              </div>
                            </div>
                          </div>
